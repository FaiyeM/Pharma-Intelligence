...
## 🎨 Application Architecture Mockups

Below are the visual architectural wireframes of the core workspaces inside **Pharma Intelligence** to guide your screenshot capture:

### 📊 View 1: Core CRM Executive Dashboard
```text
+-----------------------------------------------------------------------------------------+
| [Pharma Intelligence]   HCP ROSTER    [78 active]   EVENT RSVPs [92%]   SAMPLES [650]   |
+-----------------------------------------------------------------------------------------+
|                                                                                         |
|  +-------------------------------------+   +-----------------------------------------+  |
|  |  📍 Territory Coverage Overview     |   |  🧪 Sample Inventory Levels             |  |
|  |  ---------------------------------- |   |  -------------------------------------  |  |
|  |  Segment A Doctors: 42 (95% Met)    |   |  - Coreg (carvedilol)       [|||||..]   |  |
|  |  Segment B Doctors: 36 (80% Met)    |   |  - Januvia (sitagliptin)    [||||||.]   |  |
|  |  Segment C Doctors: 12 (40% Met)    |   |  - Entresto (sacubitril)    [|||....]   |  |
|  +-------------------------------------+   +-----------------------------------------+  |
|                                                                                         |
|  +-------------------------------------+   +-----------------------------------------+  |
|  |  📅 Medical Events & Roundtables     |   |  📞 Quick Action: Start Call Session    |  |
|  |  ---------------------------------- |   |  -------------------------------------  |  |
|  |  - Cardio Roundtable [Confirmed: 12]|   |  Select Doctor: [ Dr. Sarah Jenkins   V]|  |
|  |  - Diabetes Forum    [Invited: 24]  |   |  Select Detail: [ Entresto (HF)       V]|  |
|  |                                     |   |  [ Launch Digital eDetailing Session ]  |  |
|  +-------------------------------------+   +-----------------------------------------+  |
|                                                                                         |
+-----------------------------------------------------------------------------------------+
```

### 💻 View 2: Interactive eDetailing Slide Deck
```text
+-----------------------------------------------------------------------------------------+
| [Back to CRM]  📖 DETAILED PRESENTATION: ENTRESTO (Sacubitril/Valsartan)                 |
+-----------------------------------------------------------------------------------------+
|  +--------------------------------------+  +-----------------------------------------+  |
|  |  📉 Clinical Endpoints & Hazard Ratio |  |  🎛️ Interactive Detail Controls        |  |
|  |                                      |  |  -------------------------------------  |  |
|  |   100% | \                           |  |  Adjust Trial Dosage:                   |  |
|  |    80% |  \__  Standard Care         |  |   [ 200mg BID ]  (Recommended)           |  |
|  |    60% |     \___                    |  |   [ 100mg BID ]                         |  |
|  |    40% |         \__ Entresto        |  |                                         |  |
|  |    20% |            \______          |  |  Physician Reaction Logging:            |  |
|  |     0% +------------------------     |  |   ( ) Highly Interested   ( ) Skeptical |  |
|  |        0mo  6mo  12mo  24mo  36mo    |  |   ( ) Prefers Standard    ( ) Requested |  |
|  +--------------------------------------+  +-----------------------------------------+  |
|                                                                                         |
|  +--------------------------------------+  +-----------------------------------------+  |
|  |  📝 Live Feedback Note               |  |  🖋️ Virtual Compliance Signature        |  |
|  |  ----------------------------------  |  |  -------------------------------------  |  |
|  |  "Expressed keen interest in MACE    |  |  Drop Samples: [ 5x Starter Packs     ] |  |
|  |   reduction margins..."              |  |  Signature Pad: [~~~~~~~~~~~~~~~~~~~  ] |  |
|  +--------------------------------------+  +-----------------------------------------+  |
+-----------------------------------------------------------------------------------------+
```
...
|  |                                      |  |                                         |  |
|  |  Coaching Shortcuts:                 |  |  👤 Your Detailing Pitch:               |  |
|  |  [ Draft Clinical Follow-up Email ]  |  |  [ Yes, but our head-to-head data     ] |  |
|  |  [ Generate Pre-Visit Briefing ]     |  |                                         |  |
|  |  [ Simulate Objection Roleplay ]     |  |  [ Submit Counter-Objection Pitch ]     |  |
|  +--------------------------------------+  +-----------------------------------------+  |
+-----------------------------------------------------------------------------------------+
```
...
## 🎬 60-Second Teaser Video Script & Playbook

This cinematic, step-by-step script is optimized for recording a professional demo video. Use screen-recording software (e.g., Loom, OBS, or QuickTime) and follow this timeline:
...
Follow these guidelines to take crisp, high-fidelity screenshots for your repository:

### 🛠️ Step 1: Prepare the Environment
1. Open the application in a **separate browser window** (rather than the embedded workspace iframe) to prevent UI truncation.
2. Toggle your browser to full screen (`F11` on Windows, `Cmd + Ctrl + F` on Mac).
3. Zoom out slightly to **90%** (via `Cmd/Ctrl + -`) to allow all widgets and graphs to breathe with optimal negative space.

### 📐 Step 2: Capture States
*   **Screenshot 1: Territory CRM Hub**
    *   *Path:* Dashboard Tab (`#pharma-app-root`)
    *   *Highlights:* Ensure the colorful roundtable RSVP counts and the starter pack inventory stock bars are visible.
...
1. **Cross-Module State Synchronization**: Submitting a sample drop during an eDetailing presentation triggers real-time state changes—updating the doctor's visit counters, recording call logs, and deducting physical quantities from the live sample stock.
2. **Server-Side Clinical Intelligence**: Connects to server-side Gemini models (`process.env.GEMINI_API_KEY`) using the robust `@google/genai` SDK to evaluate clinical pitch accuracy and provide compliant life-sciences sales coaching.
3. **Interactive Trial Simulation**: Interactive data visualization controls allow representatives to toggle dose structures and view therapeutic trial endpoint correlations dynamically.
